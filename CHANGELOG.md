...
### Added
- New importable sample: **Event System** - Import it from **Package Manager → LoL Engine → Samples → Event System**. Additive; no engine API change.

### Changed
- **Object Pool** (internal perf, no API change): `IObjectPoolService.Return` now uses `TryGetComponent` instead of `GetComponent` to fetch the `PoolReference`.
- **Event System** (internal perf, no API change): `IEventManager.TriggerEvent` no longer allocates a subscriber array on every dispatch.

### Fixed
- **Cross-version (Unity 6000.5+)**: no more `FindObjectsByType` / `FindObjectsSortMode` deprecation warnings (`CS0618`), while still compiling on Unity 6000.0.
- **Fast Enter Play Mode (Unity 6.6 default)**: four static fields now reset on every Play-enter so the engine starts clean even without a domain reload.

## [1.0.0-rc.12]-2026-06-21
### Added
- **Resource Management: `ResourceLoadOptions.Optional` (bool, default `false`).** When `true`, a not-found result is treated as an expected outcome — the load returns `null` and logs only at debug level, emitting no `Warning` or `Error`. 

### Changed
- **Notification sample**: the demo now visibly shows notifications being *caught*, two ways.
- **DataPersistence sample**: the demo scene now captures real player data instead of saving an empty container.

### Fixed
- **Resource Management (buyer-visible console noise)**: loading a `Resources`-based asset no longer floods the Console with red Addressables errors. 
- **Save System**: an empty save (no registered `PersistableData`) no longer fails its own checksum and gets quarantined on load.**
- **Logger**: `LoLLogger.Configure()` no longer leaves the logger unable to identify the main thread (regression introduced with `Configure()` in 1.0.0-rc.11).
- **Asset Updater**: `DownloadAsset` no longer reports 100% progress for a *failed* download (regression introduced in 1.0.0-rc.11).
- **Audio**: corrected the `AudioPreloadManager` XML summary/field comment that still described preloaded clips as pinned with `KeepInMemory` (doc/comment-only; regression from the 1.0.0-rc.11 preload rewrite).
- **Audio**: a missing (optional) AudioMixer no longer logs Console warnings.

## [1.0.0-rc.11] - 2026-06-20
### Added
- **Setup Wizard**: auto-generates a unique `encryptionKeySeed` for save protection.
...
- **Time Management**: `TimeEvents.TimerCompleted` and `TimeEvents.ScheduledTaskExecuted` are now actually published.
...
- **Logger**: `LoLLogger.Configure(LoggerConfig)` — the file sink's settings are now reachable by users.
- **Config**: `LoLEngineConstants.SaveLoad` now surfaces the obfuscation settings (`ObfuscateSaveFiles`, `ObfuscatedSaveFileExtension`).
- **Notifications**: `INotificationService.Send(...)` gained an overload that can create persistent notifications.
- **Game State**: `IGameStateManagerService.OnAfterStateChange` — a symmetric direct C# event that passes `(previousState, newCurrentState)`.

### Deprecated
- **Game State**: `GameStateManagerService.InitializeService()` is now `[Obsolete]` — use `Initialize()`.
- `LoLEngine.Runtime.Logger.LogEvent.IsUnityLog` and `LogEvent.StackTrace` are now `[Obsolete]` (warning only).
- **`LoLEngineConfig.LogLevel`** is now `[Obsolete]` (warning only) — use `EngineLogLevel`.
- `LoLEngineConstants.ResetForTests()` is now `[Obsolete]` and renamed to `NeverEverCallThisOutsideOfTests_ResetConfig()`

### Removed
- `LoLEngine.Core.Localization.Providers.TranslationValidator` (and its `ValidationResult`) removed.

### Fixed
- **Notifications** (event-contract behavior change — see migration note): an auto-expiring notification now raises ONLY `NotificationExpired`, not both `NotificationDismissed` and `NotificationExpired`.
  - `GetNotificationHistory(count)` now returns newest-first, matching its documented contract.
  - The `MaxActiveNotifications` cap (50) is now actually enforced.
  - `Shutdown()` no longer leaks pending auto-dismiss tasks or the fallback coroutine host.
  - `Send(...)` no longer throws when called off the main thread without a TimeService scheduler.
  - `Notification.Timestamp` is now `DateTime.UtcNow` (was `DateTime.Now`) and eviction/ordering tie-breaks are deterministic.
- **Game State**: a missing optional `IEventManager` is now logged at Warning, not Error.
  - XML docs added to the public concrete and event types; the in-source usage guide was corrected.
- **Logger**: `FileSink.Emit` no longer throws `ObjectDisposedException` when a log races a concurrent `Dispose()`.
  - The default log-file path is now captured on the main thread instead of being read lazily off it.
  - `GameLogger`'s public methods now carry XML docs, and a malformed `FileSink` doc block was corrected.
- **Localization**: the runtime CSV loader now parses quoted fields correctly.** `CSVStringProvider` — the loader users' built games actually run — read the localization table line-by-line and split each line with a helper that did not carry quote state across line boundaries and never collapsed escaped doubled quotes (`""`). 
- **Dependency Checker** (editor): removed a false-positive advisory from `Tools > LoL Engine > Validate Dependencies`. 
- **Event System**: `this.EventStartListening<T>()` / `this.EventStopListening<T>()` no longer throw.
  - Corrected docs that referenced a non-existent `GameEvent.Get<T>()`.*
  - `IEventManager.GetTotalListenerCount()` no longer throws after `Shutdown()`.** It dereferenced the subscriber list with no null guard, throwing `NullReferenceException` once the service was shut down or reset; it now returns `0`, matching the null-safe `GetListenerCount<T>()`.
  - Event System sample: fixed a use-after-release in `02_EventSystemSample`.*
- **Save System (CRITICAL)**: encrypted saves no longer corrupt on synchronous load.
  - `AesEncryptionService.DecryptString` did a single `CryptoStream.Read(...)` and returned only the bytes from that one call. `CryptoStream.Read` is not guaranteed to fill the buffer, so larger or block-unaligned payloads were silently truncated in a size-dependent way (e.g. 100→96, 1000→992, 9000→8992, 65000→64992 chars), while a few buffer-aligned sizes passed by luck. Any encrypted save loaded through the synchronous path (`SaveSystem.LoadGame` → `EncryptedJsonDataSerializer.Deserialize`) at a non-aligned size was corrupted. The sync path now drains the stream in a loop, exactly mirroring the already-correct `DecryptStringAsync`.  
  - Corrupt-file quarantine no longer throws on a same-second collision.
  - Integrity checksum no longer depends on dictionary enumeration order.
  - `QuickSaveService` startup scan no longer clobbers a concurrent quick-save.
- **Object Pool**: double-return no longer hands the same instance to two callers.
  - Failing `onGet` callback no longer leaves an instance with a dirty transform.
  - `Spawn` / `SpawnWithConfig` / `PreloadPool` / `PreloadPoolWithConfig` MonoBehaviour extensions now null-guard the prefab.
  - Pool Debug window (`Tools > LoL Engine > Object Pools`) now shows the actual pools.
- **Time Management**: cancelled and disposed timers are no longer leaked.
  - `PeriodicTimer` no longer drops its final period when `repeatCount` is finite.
  - `Scheduler.SchedulePeriodic` fired one time too many.
  - The built-in "Unscaled" time channel is now genuinely unscaled.
  - Timers and the scheduler now advance on the same clock.
  - `ITimeService.DeltaTime("Global")` / `FixedDeltaTime("Global")` now honor the Global channel's own scale.
  - `DeltaTime(...)` / `ITimeChannel.DeltaTime` no longer return `0` (or a one-frame-stale value) before the frame's tick.
  - Smooth time-scale transitions no longer raise spurious `TimeEvents.TimerCompleted` events.
  - `Time("Global")` now returns the Global channel clock the scheduler uses, not Unity's wall clock.
  - Corrected the best-practice note that told buyers to use `TimeManager.unscaledUpdateMode` for real-time timing.*
- **Audio (HIGH)**: `AudioPreloadManager` now actually keeps audio resident — the old "preload" cached disposed handles and loaded nothing.
  - `PlaySoundAtPosition`, `PlaySoundAsync`, and `PlaySoundAtPositionAsync` now run the initialization guard.
  - `MusicPlaylistService` no longer leaks a preloaded clip reference when advancing tracks.
  - `FadeOut(stopAfterFade: true)` now frees a sound that was paused during the fade.
  - Public fade entry points tolerate null arguments instead of throwing.
  - Corrected the `IAudioService.SetPersistAcrossScenes` XML doc.
- **Asset Updater**: per-file download progress (`IProgress<float>`) now actually fires.
  - `VersionCatalog.HasUpdate` no longer throws on SemVer pre-release versions.
  - Local version catalog is now flushed synchronously on shutdown.
  - `InitializeAsync` / `ShutdownAsync` no longer deadlock when awaited synchronously on Unity's main thread.
- **Resource Management**: duplicate Addressables loads no longer leak the previous handle.
  - `ResourceService` cache reads are now lock-guarded.
  - `IResourceService.Release(string)` return value is now honest.
  - Batched `PreloadResources(IEnumerable<PreloadRequest>)` honors `FallbackIds` again.
  - String-id `Load<T>` / `LoadAsync<T>` now honor `ResourceLoadOptions.KeepInMemory`.
  - `GetResourceStats()` no longer throws on a null cached asset.
  - Cyclic `FallbackIds` configurations no longer recurse until the stack overflows.
- **RNG (HIGH)**: `Rng.Float()` can no longer return exactly `1.0`, honoring its documented `[min, max)` contract.
  - `Rng.Int(min, max)` no longer overflows for ranges wider than `int.MaxValue`.
- **Color helpers (HIGH)**: `ColorExtension.GetColorFromDictionary(index)` no longer throws on a negative index.*
- `TypedPoolBag.GetRemainingItems` now returns an independent snapshot.
- `ShuffleBag.Size` / `ShuffleBag.Capacity` are now read under the same lock as the mutators.
- **Localization (HIGH)**: `LocalizationConfig.stringCacheSize` is now honored.
  - `LocalizedFontConfig.GetSizeScale` no longer returns a `0` multiplier.
  - `GetPluralText(key, count, parameters)` no longer mutates the caller's dictionary.
  - `LocString.Formatters` now resets on domain reload
  - The CSV tools no longer corrupt multi-line quoted values.
- **Documentation**: corrected code snippets that did not compile against the shipped API.
- **Service Locator**: `Register`/`IsRegistered`/`Unregister` now normalize the `key` argument the same way `Get`/`TryGet` do.
  - `Register` now throws `TimeoutException` instead of silently dropping a service when it cannot acquire the write lock.
  - `Get<T>()` no longer logs a `Warning` on a miss.
  - `Require<T>()` now consults the lock-free fast cache before taking reader locks.
- **Service initialization**: fixed a guaranteed compile break in `ConfigurableServiceInitializer`.
  - External `IServiceRegistrar` discovery no longer silently drops a whole assembly that fails to fully load.
  - Removed leftover dev code from the shipped initializer.
- **Game initializer**: a second `InitializeEngine`/`InitializeEngineAsync` call while initialization is in-flight is now ignored.
  - `Detailed Logging` inspector toggle now does something.
- **Diagnostics**: `RuntimeDependencyValidator` log strings are now ASCII (`[OK]`/`[FAIL]`/`[WARN]`/`[SCAN]`).
- **Config**: fixed inert inspector attributes and added XML docs on `LoLEngineConfig`/`LoLEngineConstants`.

### Removed
- **Editor**: removed the dead `AssetVersioningTool.cs`.
- **Event System**: removed the unused `EventListenerWrapper<TOwner,TTarget,TEvent>` type.
- **Resource Management**: removed dead/never-used public types.

### Changed
- **Notifications**: `NotificationHelper` is now `internal` (was `public`).
  - `Notification` no longer carries dead Unity-serialization attributes.
- **Namespace consistency**: three public types moved into their correct namespaces before the 1.0 API freeze.
- **Editor**: package-development sample tools are now gated behind the `LOL_DEV` scripting define.
  - Quieted `SampleImportHelper` import logging.
- **Event System**: removed dead commented-out code blocks and a typo from `GameEvent`.
- **Encryption**: replaced the default obfuscation-key salt with a non-personal value.
- **Object Pool** documentation now states the system is main-thread-only.
  - Added XML documentation to the provider `ObjectPool` and `PoolReference`, and removed dead commented-out code from the object-pool sources.
- **Time Management**: `TimeManager`'s `Unscaled Update Mode` inspector toggle is now documented as not implemented.
  - Corrected the `ITimeService.GetChannel` doc and documented the provider types.
- **Audio**: removed a per-SFX diagnostic log from the orchestrator hot path.*
  - Removed two no-op "Create Audio Snapshot" menu items.
- **Resource Managemen**t: batched `PreloadResources(IEnumerable<PreloadRequest>)` no longer uses runtime reflection.
  - `GetResourceStats()` is now a side-effect-free query.
  - Removed dead commented-out scaffolding from `AssetUpdaterService` and `ResourceManagementException`.
- `ConstantsPrinter.PrintAllConstants` now also prints `const` and `static readonly` fields.
- `TypedPoolBag` draws are now O(1) per item.
- `LRUCache<TKey,TValue>` now carries XML documentation.
- **Localization**: the default `useSystemLanguage` startup log is no longer a Warning.

### Deprecated
- **Object Pool**: the `PoolReference.PoolId` setter is now `[Obsolete]`.
- `SaveConfig.SaveThumbnails` and `SaveConfig.AutoSaveOnSceneChange` are now `[Obsolete]`.
- **Audio**: `AudioService.State` and `AudioService.InitializationError` are now `[Obsolete]`.
- `Counter.GetTimeLeft` is now `[Obsolete]` in favor of `Counter.TimeLeft`.
- `LocalizationConfig.useLazyLoading` is now `[Obsolete]` — it is dead config.
- `ColorExtension.InitializeDictionary()` is now `[Obsolete]`; use `NeverEverCallThisOutsideOfTests_InitializeDictionary()` in tests.
- `LoLEngineConfig.DefaultEncryptionKey` is now `[Obsolete]` — it is dead config.

### Documentation
- Documentation accuracy pass (docs only — no API or behavior change).
  ...
  - `Architecture.md`: removed the incorrect claim that `PlayOptions` / `PlaylistPlayOptions` / `ResourceLoadOptions` use a `readonly struct` + `With*` fluent-builder pattern; they use object-initializer syntax (a mutable struct with a static `Default`, or a class with settable properties).
